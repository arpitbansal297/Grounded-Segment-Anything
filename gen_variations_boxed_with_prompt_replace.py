...
if __name__ == "__main__":
    ...
    for i in range(len(masks)):
        ...
        question += "What objects would you like to replace it with? " \
                    f"Give 5 or less relevant suggestions to replace {object_to_remove} with separated by ','. \n" \
                    f"The objects be different from {object_to_remove} and should be relevant to the image. \n" \
        ...
        for i in range(len(objects_list)):
            ...
            print(f"Replacing {object_to_remove} with {object_to_replace_with}")
            question = f"The description of the image is given by the following caption. \n"
            question += f"\"{original_caption}\". \n"
            question += f"We first removed {object_to_remove} completely from the image" \
                        f" and then replaced {object_to_remove} with {object_to_replace_with} by inpainting. \n" \
                        f"Hence there is no information about {object_to_remove} in the image. \n"
            question += "Based on this change, imagine and write a new caption for this modified image. \n"
            print(question)
            response = text_model.generate_content(question,
                                                   generation_config=genai.types.GenerationConfig(temperature=0.0))
            response.resolve()
            ...
            question = f"The description of the image is given by the following caption. \n"
            question += f"\"{new_caption}\". \n"
            question += f"Rewrite the caption to be less than 70 words and when rewriting you must keep" \
                        f" {object_to_replace_with} in the caption.\n"
            response = text_model.generate_content(question,
                                                   generation_config=genai.types.GenerationConfig(temperature=0.0))
            response.resolve()
            new_caption = response.text
            ...
            prompt = f"High quality, detailed, 8k uhd. {new_caption}"
            ...
            inpaint_image = pipeline(prompt=prompt, negative_prompt=negative_prompt, negative_prompt_2=negative_prompt,
                                     ...
                                     num_inference_steps=200, height=height, width=width).images[0]
            print(inpaint_image)
            inpaint_image.save(os.path.join(masked_directory, f"inpaint_replace_{object_to_remove}_"
                                                              f"with_{object_to_replace_with}.jpg"))
            ...
            new_image_pil, _ = load_image(os.path.join(masked_directory, f"inpaint_replace_{object_to_remove}_"
                                                                         f"with_{object_to_replace_with}.jpg"))
            question = caption_question
            gen_caption = vision_model.generate_content([question, new_image_pil])
            gen_caption.resolve()
            gen_caption = gen_caption.text
            print(gen_caption)
            ...
            prompt = f"High quality, detailed, 8k uhd. There is {object_to_replace_with} in the image."
            print(f"Prompt: {prompt}")
            ...
            print(inpaint_image)
            inpaint_image.save(os.path.join(masked_directory, f"inpaint_replace_{object_to_remove}_"
                                                              f"with_{object_to_replace_with}_simple.jpg"))
            ...
            new_image_pil, _ = load_image(os.path.join(masked_directory, f"inpaint_replace_{object_to_remove}_"
                                                                         f"with_{object_to_replace_with}_simple.jpg"))
            question = caption_question
            gen_caption_simple = vision_model.generate_content([question, new_image_pil])
            gen_caption_simple.resolve()
            gen_caption_simple = gen_caption_simple.text
            print(gen_caption_simple)



            # init_image = load_image_diffusers(os.path.join(masked_directory, f"inpaint_replace_{object_to_remove}_"
            #                                                                  f"with_{object_to_replace}.jpg"))
            # mask_image = load_image_diffusers(os.path.join(masked_directory, f"new_mask.png"))
            #
            #
            # inpaint_image = refiner_pipeline(prompt=prompt, negative_prompt=negative_prompt, negative_prompt_2=negative_prompt,
            #                                  image=init_image, mask_image=mask_image,
            #                                  generator=generator_1, guidance_scale=7.5, denoising_start=0.2,
            #                                  height=height, width=width).images[0]
            # inpaint_image.save(os.path.join(masked_directory, f"inpaint_replace_{object_to_remove}_"
            #                                                   f"with_{object_to_replace}_refined.jpg"))
            #
            #
            #
            # new_image_pil, _ = load_image(os.path.join(masked_directory, f"inpaint_replace_{object_to_remove}_"
            #                                                              f"with_{object_to_replace}_refined.jpg"))
            # question = caption_question
            # gen_caption_refined = vision_model.generate_content([question, new_image_pil])
            # gen_caption_refined.resolve()
            # gen_caption_refined = gen_caption_refined.text
            # print(gen_caption_refined)
            #
            # del inpaint_image
            # del mask_image
            # del init_image
            ...
            with open(f"{masked_directory}/captions_replace_{object_to_remove}_with_{object_to_replace_with}.txt",
                ...
                # new_caption
                f.write(f"New Caption modified by text gen: \n")
                f.write(new_caption)
                f.write("\n")
                f.write("\n")

                # gen_caption
                f.write(f"Caption caption by vision gen from inpainted image: \n")
                f.write(gen_caption)
                f.write("\n")
                f.write("\n")

                # gen_caption simple
                f.write(f"Caption caption by vision gen from simple inpainted image: \n")
                f.write(gen_caption_simple)
                f.write("\n")
                f.write("\n")

                # # gen_caption refined
                # f.write(f"Caption caption by vision gen from inpainted and refined image: \n")
                # f.write(gen_caption_refined)
                # f.write("\n")
